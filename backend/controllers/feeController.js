...
const DEFAULT_FEES = (studentId, studentName, semester) => {
  ...
  const overD = new Date(now); overD.setDate(overD.getDate() - 10);
...
exports.getFees = async (req, res) => {
  try {
    ...
    if (req.user.role === "student" && fees.length === 0) {
      await Fee.insertMany(DEFAULT_FEES(req.user._id, req.user.name, req.user.semester));
      fees = await Fee.find({ studentId: req.user._id })
        .populate("studentId", "name rollNo class department semester")
        .sort({ dueDate: 1 });
    }
...
exports.createFee = async (req, res) => {
  try {
    res.status(201).json(await Fee.create(req.body));
